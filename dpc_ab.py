...
def dpc_with_alpha_beta(data, alpha, beta, distance_threshold):
    ...
    densities = compute_density(data, alpha, beta)
...
accuracy_DPC = accuracy_score(original_labels, labels_pred)
n_clusters = 2
n_runs = 30
accuracies_kmeans = []
for _ in range(n_runs):
    kmeans = KMeans(n_clusters=n_clusters)
    labels_kmeans = kmeans.fit_predict(normalize_to_distribution(data))  # 使用归一化数据
    acc = accuracy_score(original_labels, labels_kmeans)
    accuracies_kmeans.append(acc)
t_stat, p_value = ttest_1samp(accuracies_kmeans, popmean=accuracy_DPC)
...
# def plot_alpha_beta_comparisons(data, alpha_values, beta_values):
#     data=np.asarray(data)
...
#                     dist = alpha_beta_divergence(data[i], data[j], alpha, beta)
#                     distances.append(dist)
#             plt.hist(distances, bins=30, alpha=0.5, label=f'α={alpha}, β={beta}')
    
#     plt.xlabel('Alpha-Beta Divergence')
#     plt.ylabel('Frequency')
#     plt.title('Alpha-Beta Divergence Comparison')
#     plt.legend()
#     plt.show()

# # 调用函数可视化
# plot_alpha_beta_comparisons(data, alpha_values=[0.5, 1.0, 1.5], beta_values=[0.5, 1.0, 1.5])
