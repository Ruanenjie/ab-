...
def compute_prominence(cluster_data, alpha, beta):
    """
    计算聚类质心的显著性值
    """
    n_samples = len(cluster_data)
    avg_distance = np.mean([alpha_beta_divergence(cluster_data[i], cluster_data[j], alpha, beta) for i in range(n_samples) for j in range(i + 1, n_samples)])
    return avg_distance
...
def dpc_with_alpha_beta(data, alpha, beta, distance_threshold):
    ...
    for i in sorted_indices:
        if clusters[i] == -1:  # 如果点i未被分配到任何簇
            ...
            cluster_prominence = compute_prominence(cluster_points, alpha, beta)
            dynamic_beta = (1 - cluster_prominence) * 0.5  # 根据显著性动态调整beta
            ...
            for j in range(len(data)):
                if clusters[j] == -1:  # 点j尚未分类
                    # 检查与当前簇内所有点的相似度
                    cluster_points = [data[k] for k in range(len(data)) if clusters[k] == cluster_id]
                    if all(alpha_beta_divergence(data[j], cp, alpha, dynamic_beta) < distance_threshold for cp in cluster_points):
...
clusters, centers = dpc_with_alpha_beta(data, alpha, beta, distance_threshold)
